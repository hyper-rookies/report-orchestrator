...
test("yields actionGroupOutput event for observation trace", async () => {
  const client = new BedrockAgentClient(
    makeSdkMock([
      {
        trace: {
  ...
  const events = await collectEvents(client);
  expect(events).toHaveLength(1);
  expect(events[0]).toMatchObject({ type: "actionGroupOutput", actionGroup: "query" });
});
