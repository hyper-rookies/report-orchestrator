...
**SQL build rules** (enforced by query-lambda, not by the caller):
...
5. A `WHERE dt BETWEEN :start AND :end` clause is always injected.
6. DML statements (`INSERT`, `UPDATE`, `DELETE`, `DROP`, `TRUNCATE`, `CREATE`, `ALTER`) are rejected with `SCHEMA_VIOLATION`.
...
| `workgroup` | string | yes | Fixed per environment; default `hyper-intern-m1c-wg` |
| `database` | string | yes | Fixed per environment; default `hyper_intern_m1c` |
| `outputLocation` | string (S3 URI) | yes | If not provided by caller, query-lambda uses env var `ATHENA_OUTPUT_LOCATION` |
...
All Action Group Lambdas return errors in this shape. HTTP status is `200`; the Bedrock Agent reads the response body to detect errors.
...
| Code | Action Group | Retryable | Meaning |
|---|---|---|---|
| `QUERY_TIMEOUT` | query | false | Athena execution exceeded `timeoutSeconds` |
| `ATHENA_FAILED` | query | false | Athena returned `FAILED` state |
| `SCHEMA_VIOLATION` | query | false | SQL references denied view, table, or column |
| `DML_REJECTED` | query | false | SQL contains DML statement |
| `ALIGNMENT_ERROR` | analysis | false | `groupBy` columns not found in input rows |
| `INVALID_CHART_TYPE` | viz | false | `chartType` not in allowed enum |
| `MISSING_AXIS` | viz | false | `xAxis`/`yAxis` required but absent for bar/line |
| `GUARDRAIL_BLOCKED` | orchestrator | false | Bedrock guardrail rejected the request |
| `UNKNOWN` | any | false | Unclassified internal error |
